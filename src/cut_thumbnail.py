...
def _check_random_cut_thumbnail_args(thumbnail_area, resize_method, augmentation_ratio, bernoulli_mix):
    """
...
def _calculate_thumbnail_size(image_size, thumbnail_area):
    """
...
def random_cut_thumbnail(
    ...
    _check_random_cut_thumbnail_args(thumbnail_area, resize_method, augmentation_ratio, bernoulli_mix)
    ...
    thumbnail_size = _calculate_thumbnail_size(image_shape[1:3], thumbnail_area)
    thumbnails = tf.image.resize(images, thumbnail_size, method=resize_method)
...
class RandomCutThumbnail(tf.keras.Layer):
    """
    This keras layer implements the "cut thumbnail" data augmentation technique.
    ...
    Refer to the docstring of the random_cut_thumbnail() function for an explanation
