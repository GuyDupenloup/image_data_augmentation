...
from dataaug_utils import (
    check_dataaug_function_arg, rescale_pixel_values, gen_patch_contents, mix_augmented_images)


def _check_random_grid_mask_args(
    unit_length, masked_length_ratio, fill_method, pixels_range, augmentation_ratio, bernoulli_mix):
    """
...
def _generate_grid_mask(image_shape, unit_length, masked_ratio):
    """
...
def random_grid_mask(
    ...
    _check_random_grid_mask_args(unit_length, masked_ratio, fill_method, pixels_range, augmentation_ratio, bernoulli_mix)
    ...
    mask = _generate_grid_mask(image_shape[:3], unit_length, masked_ratio)

    # Generate the contents of the erased areas
    unit_contents = gen_patch_contents(images, fill_method)
...
class RandomGridMask(tf.keras.Layer):
    ...
    Refer to the docstring of the random_grid_mask() function for an explanation
